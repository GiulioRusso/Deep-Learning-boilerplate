...
def min_max_statistics(dataset: Dataset) -> Tuple[int, int]:
    ...
    for i in tqdm(range(dataset.__len__()), desc="processing data..."):
        # sample
        sample = dataset[i]
...
def read_min_max_statistics(statistics_path: str) -> dict:
    """
    Read (min, max) statistics from CSV file
    ...
    statistics = read_csv(filepath_or_buffer=statistics_path, usecols=["DATASET", "MIN", "MAX"])
